...
class MainActivity : Activity() {
    ...
    // Synthetic-positive sample/generation state is keyed by source ("f5" /
    // "zonos") so the two Review cards never clobber each other's loaded spread
    // or in-flight generation job. Slug is tracked per source so a card only
    // shows samples that belong to the project currently on screen.
    private val syntheticSlug = HashMap<String, String?>()
    private val syntheticSamples = HashMap<String, List<SyntheticSample>>()
    private val loadingSynthetic = HashMap<String, Boolean>()
    // Per-source synthetic generation, kicked off from the Review card and polled
    // while the server runs the batch. Slug scopes the status line to the project
    // being generated for that source.
    private val synthGenSlug = HashMap<String, String?>()
    private val synthGenerating = HashMap<String, Boolean>()
    private val synthGenMessage = HashMap<String, String?>()
    ...
    private fun renderRecordPage(project: WakeWordProject?) {
        ...
        // Hard negatives carry a prompt (near-miss phrases to read). F5 voice
        // cloning now seeds from your real positive takes, so there is no separate
        // enrollment recorder.
            ...
            ReviewSection.Synthetic -> {
                workspace.addView(
                    syntheticSamplesCard(
                        ...
                        SYNTH_SOURCE_F5,
                        "F5 voice clones",
                        "Zero-shot voice clones generated by F5-TTS from your recorded positives.",
                    ).withTop(dp(12)),
                )
                workspace.addView(
                    syntheticSamplesCard(
                        project,
                        SYNTH_SOURCE_ZONOS,
                        "Zonos voice clones (prosody levers)",
                        "Voice clones generated by Zonos, with controllable prosody, from your recorded positives.",
                    ).withTop(dp(12)),
                )
            }
        }
    ...
    private fun trainingFormCard(project: WakeWordProject): View {
        ...
        val zonosInput = EditText(this).apply {
            hint = "Zonos voice-cloned clips"
            isSaveEnabled = false
            setSingleLine()
            inputType = InputType.TYPE_CLASS_NUMBER
            imeOptions = EditorInfo.IME_ACTION_DONE
        ...
        return card().apply {
            ...
            addView(
                text(
                    "Four sources feed the model's positives: the built-in Kokoro TTS pool, F5 voice-cloned clips, Zonos voice-cloned clips (both generated on the server at train time from your recordings), and your own real positives replicated by the boost. The calculator sums them.",
                    12f,
                    mutedColor(),
                ).withTop(dp(4)),
            )
            ...
            addView(text("Zonos voice-cloned clips", 15f, mutedColor()).withTop(dp(14)))
            addView(
                text(
                    "A second voice-clone source (controllable prosody), also cloned from your recorded positives and generated fresh at train time.",
                    11f,
                    mutedColor(),
                ).withTop(dp(2)),
            )
            addView(zonosInput.withTop(dp(6)))
    ...
    // Zonos voice-cloned positives generated at train time. 0 = none (opt-in).
    private fun savedTrainZonosCount(): Int {
        return getSharedPreferences(SYNC_PREFS, Context.MODE_PRIVATE)
            .getInt(KEY_TRAIN_ZONOS_COUNT, 0)
            .coerceIn(0, 50_000)
    }
     ...
     */
    private fun syntheticSamplesCard(
        project: WakeWordProject,
        source: String,
        title: String,
        blurb: String,
    ): View {
        val mine = if (syntheticSlug[source] == project.slug) {
            syntheticSamples[source] ?: emptyList()
        } else {
            emptyList()
        }
        val loading = loadingSynthetic[source] == true
        return card().apply {
            addView(
                LinearLayout(this@MainActivity).apply {
                    orientation = LinearLayout.HORIZONTAL
                    gravity = Gravity.CENTER_VERTICAL
                    addView(
                        text("$title  ${mine.size}", 18f, textColor(), Typeface.BOLD).apply {
                            layoutParams = LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f)
                        },
                    )
                    addView(actionButton(if (loading) "Loading…" else "Load sample", ButtonStyle.Secondary) {
                        if (!loading) loadSyntheticSamples(project, source)
                    })
                },
            )
            addView(
                text(
                    "$blurb A spread-out sample of the full batch — tap one to hear how it sounds. " +
                        "Training regenerates these on the server; delete them to force a fresh batch next run.",
                    13f,
                    mutedColor(),
                ).withTop(dp(4)),
            )
            val generatingThis = synthGenerating[source] == true && synthGenSlug[source] == project.slug
                ...
                LinearLayout(this@MainActivity).apply {
                    ...
                    addView(
                        actionButton(
                            ...
                            ButtonStyle.Primary,
                        ) { if (!generatingThis) startSyntheticGeneration(project, source, previewCount) }
                            .apply { isEnabled = !generatingThis }.weight1(),
                    )
                    addView(
                        actionButton("Delete all", ButtonStyle.Secondary) {
                            confirmDeleteSynth(project, source)
                        }.weight1().withLeft(dp(8)),
                    )
                }.withTop(dp(8)),
            )
            ...
            if (synthGenSlug[source] == project.slug && synthGenMessage[source] != null) {
                addView(text(synthGenMessage[source]!!, 13f, textColor()).withTop(dp(8)))
            }
            if (mine.isEmpty()) {
                ...
                mine.forEach { sample ->
                    addView(syntheticSampleRow(sample, source).withTop(dp(8)))
                }
            }
        }
    }
    ...
    private fun syntheticSampleRow(sample: SyntheticSample, source: String): View {
        val playbackKey = "synthetic:$source:${sample.id}"
        val playing = activePlaybackKey == playbackKey && player?.isPlaying == true
        return LinearLayout(this).apply {
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER_VERTICAL
            ...
            addView(actionButton(if (playing) "❚❚ Pause" else "▶ Play", ButtonStyle.Secondary) {
                playSyntheticSample(sample, source)
            })
        }
    }
    ...
    private fun enrollmentDetailCard(project: WakeWordProject, recording: BulkRecording): View {
        ...
        return card().apply {
            ...
            addView(
                text(
                    "A legacy voice-enrollment take. Enrollment has been retired — F5 voice " +
                        "cloning now seeds from your real positive takes, and synthetic clips are " +
                        "generated at train time. You can play this back or delete it.",
                    13f,
                    mutedColor(),
                ).withTop(dp(10)),
            )
     ...
     */
    private fun startSyntheticGeneration(
        project: WakeWordProject,
        source: String,
        count: Int = SYNTH_PREVIEW_COUNT,
    ) {
        val serverUrl = savedServerUrl()
        if (serverUrl.isBlank()) {
            statusMessage = "Set a sync server URL in Settings first"
        ...
        synthGenSlug[source] = project.slug
        synthGenerating[source] = true
        synthGenMessage[source] = "Starting generation of $count samples…"
        render()
        Thread {
            try {
                val client = BundleSyncClient(serverUrl)
                client.startSyntheticGeneration(project.slug, count, source)
                // Poll until the server reports the run finished. Clone batches take
                // minutes, so a slow poll keeps traffic light.
                while (true) {
                    Thread.sleep(4_000)
                    val status = client.syntheticGenerationStatus(project.slug, source)
                    if (status.running) {
                        runOnUiThread {
                            if (synthGenSlug[source] == project.slug) {
                                synthGenMessage[source] = "Generating $count samples in your voice — this runs on the server…"
                                render()
                            }
                        }
                        continue
                    ...
                    val samples = try {
                        client.loadSyntheticSamples(project.slug, source)
                    } catch (_: Exception) {
                        syntheticSamples[source] ?: emptyList()
                    }
                    runOnUiThread {
                        synthGenerating[source] = false
                        syntheticSlug[source] = project.slug
                        syntheticSamples[source] = samples
                        synthGenMessage[source] = when {
                            status.error != null -> "Generation failed: ${status.error}"
                            status.wrote > 0 -> "Generated ${status.wrote} samples. Open Review → Synthetic to hear them."
                            else -> "Generation finished. Open Review → Synthetic."
                        }
                        statusMessage = synthGenMessage[source] ?: ""
                        render()
                    }
                    break
                }
                ...
                runOnUiThread {
                    synthGenerating[source] = false
                    synthGenMessage[source] = error.message ?: "Generation failed"
                    statusMessage = synthGenMessage[source] ?: ""
                    render()
                }
            }
    ...
    private fun confirmDeleteSynth(project: WakeWordProject, source: String) {
        val label = if (source == SYNTH_SOURCE_ZONOS) "Zonos" else "F5"
        AlertDialog.Builder(this)
            .setTitle("Delete all $label clips?")
            .setMessage(
                "Removes every $label voice-cloned positive for “${project.phrase}” from the server. " +
                    "Training will regenerate them next run. This cannot be undone.",
            )
            .setNegativeButton("Cancel", null)
            .setPositiveButton("Delete") { _, _ -> deleteSyntheticSamples(project, source) }
            .show()
    }

    private fun deleteSyntheticSamples(project: WakeWordProject, source: String) {
        val serverUrl = savedServerUrl()
        if (serverUrl.isBlank()) {
            statusMessage = "Set a sync server URL in Settings first"
            render()
        ...
        // Clear the loaded spread immediately so the card empties at once.
        if (syntheticSlug[source] == project.slug) syntheticSamples[source] = emptyList()
        statusMessage = "Deleting synthetic clips…"
        render()
        Thread {
            try {
                val deleted = BundleSyncClient(serverUrl).deleteSyntheticSamples(project.slug, source)
                runOnUiThread {
                    statusMessage = "Deleted $deleted synthetic clip${if (deleted == 1) "" else "s"}"
                    render()
                }
            } catch (error: Exception) {
                runOnUiThread {
                    statusMessage = error.message ?: "Delete synthetic failed"
                    render()
                }
            }
        }.start()
    ...
    private fun loadSyntheticSamples(project: WakeWordProject, source: String) {
        val serverUrl = savedServerUrl()
        if (serverUrl.isBlank()) {
            statusMessage = "Server URL required"
            currentPage = AppPage.Settings
        ...
        loadingSynthetic[source] = true
        statusMessage = "Loading synthetic samples for ${project.slug}"
        render()
        Thread {
            try {
                val samples = BundleSyncClient(serverUrl).loadSyntheticSamples(project.slug, source)
                runOnUiThread {
                    syntheticSlug[source] = project.slug
                    syntheticSamples[source] = samples
                    loadingSynthetic[source] = false
                    statusMessage = if (samples.isEmpty()) {
                        "No synthetic clips yet. Generate them from the card or at train time."
                    } else {
    ...
    private fun playSyntheticSample(sample: SyntheticSample, source: String) {
        val playbackKey = "synthetic:$source:${sample.id}"
        player?.let { current ->
            if (activePlaybackKey == playbackKey) {
                if (current.isPlaying) current.pause() else current.start()
    ...
    private companion object {
        ...
        // The two synthetic-positive sources the server exposes behind ?source=.
        // F5 = zero-shot voice clone; Zonos = controllable-prosody voice clone.
        const val SYNTH_SOURCE_F5 = "f5"
        const val SYNTH_SOURCE_ZONOS = "zonos"
