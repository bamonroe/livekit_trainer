...
class MainActivity : Activity() {
    ...
    private fun syntheticSamplesCard(project: WakeWordProject): View {
        ...
        return card().apply {
            ...
            addView(
                text(
                    "Voice-cloned positives generated by F5-TTS from your recorded positives. A spread-out " +
                        "sample of the full batch — tap one to hear how it sounds. Training regenerates " +
                        "these on the server; delete them to force a fresh batch next run.",
                    13f,
                    mutedColor(),
                ).withTop(dp(4)),
            )
    ...
    private fun confirmDeleteSynth(project: WakeWordProject) {
        AlertDialog.Builder(this)
            .setTitle("Delete all synthetic clips?")
            .setMessage(
                "Removes every F5 voice-cloned positive for “${project.phrase}” from the server. " +
                    "Training will regenerate them next run. This cannot be undone.",
            )
            .setNegativeButton("Cancel", null)
            .setPositiveButton("Delete") { _, _ -> deleteSyntheticSamples(project) }
            .show()
    }

    private fun deleteSyntheticSamples(project: WakeWordProject) {
        val serverUrl = savedServerUrl()
        if (serverUrl.isBlank()) {
            statusMessage = "Set a sync server URL in Settings first"
            render()
        ...
        if (syntheticSlug == project.slug) syntheticSamples = emptyList()
        statusMessage = "Deleting synthetic clips…"
        render()
        Thread {
            try {
                val deleted = BundleSyncClient(serverUrl).deleteSyntheticSamples(project.slug)
                runOnUiThread {
